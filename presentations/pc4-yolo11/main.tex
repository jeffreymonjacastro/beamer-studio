...
% ── SLIDE 7: SPFF ──────────────────────────────────────────────
\begin{frame}{YOLOv11: Spatial Pyramid Pooling Fast Block}
  \begin{columns}[c]
...
% ── SLIDE 8: C2PSA ──────────────────────────────────────────────
\begin{frame}{YOLOv11: Cross Stage Partial with Spatial Attention Block}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \centering
      \vspace{-0.15cm}
      \begin{tikzpicture}[
        ...
        \node[layer] (attn) at (-1.65,1.70) {Attention};
        \node[layer] (p_concat1) at (-1.65,0.95) {Concat};
        \node[layer] (ffn) at (-1.65,0.20) {FFN};
        \node[layer] (p_conv1) at (-1.65,-0.55) {Conv 1x1};
        \node[layer] (p_conv2) at (-1.65,-1.30) {Conv 1x1};
        \node[layer] (p_concat2) at (-1.65,-2.05) {Concat};

        \draw[arr] (attn) -- (p_concat1);
        \draw[arr] (p_concat1) -- (ffn);
        \draw[arr] (ffn) -- (p_conv1);
        \draw[arr] (p_conv1) -- (p_conv2);
        \draw[arr] (p_conv2) -- (p_concat2);
        \draw[skip] (attn.east) -- ++(0.52,0) |- (p_concat1.east);
        \draw[skip] (ffn.east) -- ++(0.52,0) |- (p_concat2.east);
        ...
        \draw[arr] (psa_box.east) -- (c2_box.west);
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.5\textwidth}
      \small
...
\begin{frame}{YOLOv11: Neck Step}
  ...
  \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[
      ...
      \draw[arr] (sppf) -- (c2psa);
      \draw[route] (c2psa.east) -- ++(1.05,0) |- (up1.west);
      ...
      \draw[route] (c3k2_2.east) -- ++(1.85,0) |- (r_cbs1.north);
      ...
      \draw[route] (bb2.east) -- node[above,font=\small\bfseries,text=utecblue!85] {80x80} ++(1.45,0) |- (concat2.west);
      \draw[route] (bb3.east) -- node[above,font=\small\bfseries,text=utecblue!85] {40x40} ++(1.95,0) |- (concat1.west);
      \draw[route] (bb4.east) -- node[above,font=\small\bfseries,text=utecblue!85] {20x20} ++(1.55,0) |- (sppf.west);

      \draw[route] (c2psa.east) -- ++(3.15,0) |- (r_concat2.west);
      \draw[route] (c3k2_1.east) -- ++(2.20,0) |- (r_concat1.west);
      ...
      \draw[route] (c3k2_2.east) -- ++(0.55,0) -- ++(0,0.62) -- ++(1.36,0) -- ++(0,-1.87) -- (d3.west);
